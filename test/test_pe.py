...
def from_fixed(val, frac_bits=8):
    if type(val) is cocotb.binary.BinaryValue:
        val = int(val)
    if val >= (1 << 15):
        val -= (1 << 16)
...
async def test_pe(dut):
    ...
    assert from_fixed(to_fixed(-3.3984375)) == -3.3984375, "to_fixed or from_fixed is broken"
    ...
    assert dut.pe_accept_w_out.value == 1, f"dut.pe_accept_w_out was {dut.pe_accept_w_out.value}, expected 1"
    ...
    assert dut.pe_psum_out.value == to_fixed(21.203125), f"dut.pe_psum_out was {from_fixed(dut.pe_psum_out.value)}, expected 21.203125"
    ...
    assert dut.weight_reg_active.value == to_fixed(5.75), f"dut.weight_reg_active was {from_fixed(dut.weight_reg_active.value)}, expected 5.75"
    assert dut.pe_weight_out.value == to_fixed(0.0), f"dut.pe_weight_out was {from_fixed(dut.pe_weight_out.value)}, expected 0.0"
    assert dut.pe_psum_out.value == to_fixed(-19.5390625), f"dut.pe_psum_out was {from_fixed(dut.pe_psum_out.value)}, expected -19.5390625"
